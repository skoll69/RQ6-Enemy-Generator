...
if command -v lsof >/dev/null 2>&1; then
  ...
  if [ -n "$PIDS" ]; then
    echo "[start_db_simple] Detected listeners on 3306: $PIDS" >&2
    kill $PIDS >/dev/null 2>&1 || true
    sleep 0.3
    STILL=$(lsof -t -iTCP:3306 -sTCP:LISTEN 2>/dev/null | sort -u || true)
    if [ -n "$STILL" ]; then echo "[start_db_simple] Forcing kill for: $STILL" >&2; kill -9 $STILL >/dev/null 2>&1 || true; fi
  fi
elif command -v netstat >/dev/null 2>&1; then
  echo "[start_db_simple] netstat available but cannot identify PIDs reliably on macOS; if 3306 is busy, consider: sudo lsof -iTCP:3306 -sTCP:LISTEN -Pn" >&2
fi
